...
category = 1 #number of variables, i.e., stream function
...
def ComputeCost_SE(X,Y):
    ...
    d4si_dx4_in = tf.gather(tf.reshape(backend.gradients(backend.gradients(backend.gradients(backend.gradients(Y[0][:,:,0], X)[0][:,:,0], X)[0][:,:,0], X)[0][:,:,0], X)[0][:,:,0],[-1]),pose_interior_p)

    dsi_dy_in =   tf.gather(tf.reshape(backend.gradients(Y[0][:,:,0], X)[0][:,:,1],[-1]),pose_interior_p)
    d2si_dy2_in = tf.gather(tf.reshape(backend.gradients(backend.gradients(Y[0][:,:,0], X)[0][:,:,1], X)[0][:,:,1],[-1]),pose_interior_p)
    d3si_dy3_in = tf.gather(tf.reshape(backend.gradients(backend.gradients(backend.gradients(Y[0][:,:,0], X)[0][:,:,1], X)[0][:,:,1], X)[0][:,:,1],[-1]),pose_interior_p)
    d4si_dy4_in = tf.gather(tf.reshape(backend.gradients(backend.gradients(backend.gradients(backend.gradients(Y[0][:,:,0], X)[0][:,:,1], X)[0][:,:,1], X)[0][:,:,1], X)[0][:,:,1],[-1]),pose_interior_p)
    
    d4si_dy2_dx2_in = tf.gather(tf.reshape(backend.gradients(backend.gradients(backend.gradients(backend.gradients(Y[0][:,:,0], X)[0][:,:,1], X)[0][:,:,1], X)[0][:,:,0], X)[0][:,:,0],[-1]),pose_interior_p)
